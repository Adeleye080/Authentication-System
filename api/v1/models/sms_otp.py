...
class SMSOtpCodes(Base):
    __tablename__ = "auth_sms_otp_codes"
    ...
    def create(cls, code: str, user_id: str):
        """Use to create SMS OTP instance"""
        code_hash = hashlib.sha256(f"{code}:{user_id}".encode()).hexdigest()
        return cls(user_id=user_id, code_hash=code_hash)

    def __repr__(self):
        return f"<UserSMSCode(id={self.id}, user_id={self.user_id}, created_at={self.created_at.isoformat()})>"
